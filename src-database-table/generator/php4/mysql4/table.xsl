...
class Db]]><xsl:value-of select="@classname"/><![CDATA[Filter
{
	...
	var $]]><xsl:value-of select="@classname"/><![CDATA[Filter ;
...
<!-- == == == == == == == == == Bean generation == == == == == == == == == -->
	...
	{]]><xsl:apply-templates select="model/property" mode="FieldInitializer"/><![CDATA[
		$this->]]><xsl:value-of select="@classname"/><![CDATA[Filter = new Db]]><xsl:value-of select="@classname"/><![CDATA[Filter() ;
	}
...
<!-- == == == == == == == == == Filter generation == == == == == == == == == -->
	...
	{
		$this->]]><xsl:value-of select="@classname"/><![CDATA[Filter = $this ;
...
<!-- == == == == == == == == == Table operation generation == == == == == == == == == -->
	...
	function getList($filter = null, $rangeStart = -1, $rangeLength = 0)
	{
		...
		if (null == $filter) //default selector
		{
			...
			$sql = $this->buildSqlSelect ($this->myTable, $columnList, $whereList, $whereOperator, $orderList, $groupList) ;
		}
	...
	function update($row)
	{
		...
		$whereList = array() ;
		$whereList[] = $this->buildCondition($this->myModel[']]><xsl:value-of select="model/@sidColumn"/><![CDATA['], $row->get]]><xsl:value-of select="model/@sidName"/><![CDATA[(), 'eq', ']]><xsl:value-of select="model/@sidType"/><![CDATA[') ;
...
<!-- == == == == == == == == == Common == == == == == == == == == -->
	...
	<xsl:template match="enum" mode="FieldInitializer"><![CDATA[
		$this->set]]><xsl:value-of select="@name"/><![CDATA[(]]>$this-><xsl:value-of select="../../@classname"/>Filter-><xsl:value-of select="@name"/>_<xsl:value-of select="@defaultValue"/><![CDATA[) ;]]></xsl:template>
	...
	<xsl:template match="selectors" mode="buildSelectors">
		 ...
		 *==================*/
			...
			<xsl:with-param name="variable_name" select="$filter_name"/>
		</xsl:apply-templates><![CDATA[
		{
			$columnList = array() ;
	...
	<xsl:template match="condition" mode="buildIfConditions">
		...
		<xsl:variable name="operator"><xsl:choose>
			...
			<xsl:when test="@enum = 'true'"><xsl:value-of select="concat('$', $variable_name,'->get',@name,'()',$operator,'$this->',/table/@classname,'Filter->',@name,'_',@value)"/></xsl:when>
