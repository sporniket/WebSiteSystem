 ...
 *mm:minute
 *ss:second
 * or string (should be iso 8601 format)
 */
class DatabaseTable
{
	//constants
	...
	function buildInsertQuery($tableName, $columnList, $valueList)
	{
		...
		$sql .= $this->buildInsertValuesClause($columnList, $valueList) ;
		return $sql ;
	}
	...
	function buildUpdateQuery($tableName, $columnList, $valueList, $whereList, $whereOperator)
	{
		...
		$sql .= $this->buildUpdateSetClause($columnList, $valueList) ;
		if ((0 < count($whereList)))
		{
			$sql .= ' WHERE ' ;
			$sql .= $this->buildConditionGroupClause($whereList, $whereOperator) ;
		}
		return $sql ;
	}
	...
	function buildSqlSelect ($tableName, $columnList, $whereList, $whereOperator, $orderList, $groupList)
	{
		...
		$sql .= $this->buildOrderByClause($orderList) ;
		$sql .= $this->buildGroupByClause($groupList) ;
		return $sql ;
	}
	...
	function buildCondition($colName,$colValue,$operator='eq',$colType='string')
	{
		...
		return $this->buildExpression($colName, $encodedValue, $operator) ;
	}
	...
	{
		$encodedValue = '' ;
		if ($type == $this->DbTable->TYPE_INTEGER)
		{
			$encodedValue = (integer)$value ;
		}
		else if ($type == $this->DbTable->TYPE_UNSIGNED_INTEGER)
		{
			$encodedValue = (integer)$value ;
		}
		else if ($type == $this->DbTable->TYPE_STRING)
		{
			$encodedValue = '\''.addslashes($value).'\'' ;
		}
		else if ($type == $this->DbTable->TYPE_CHAR)
		{
			$encodedValue = '\''.addslashes($value).'\'' ;
		}
		else if ($type == $this->DbTable->TYPE_TEXT)
		{
			$encodedValue = '\''.addslashes($value).'\'' ;
		}
		else if ($type == $this->DbTable->TYPE_DATETIME)
		{
	 ...
	 *For the operator $this->DbTable->OPERATOR_IN, the second member is put inside parenthesis.
	 *No sanity checks is performed .
	 *static.
	...
	{
		if ($operator == $this->DbTable->OPERATOR_EQUAL)
		{
			return ' '.$left.' = '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_GREATER_OR_EQUAL)
		{
			return ' '.$left.' >= '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_LESSER_OR_EQUAL)
		{
			return ' '.$left.' <= '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_GREATER)
		{
			return ' '.$left.' > '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_LESSER)
		{
			return ' '.$left.' < '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_IN)
		{
			return ' '.$left.' in ('.$right.') ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_LIKE)
		{
			return ' '.$left.' like '.$right.' ' ;
		}
		//NOT
		else if ($operator == $this->DbTable->OPERATOR_NOT_EQUAL)
		{
			return ' not '.$left.' = '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_NOT_GREATER_OR_EQUAL)
		{
			return ' not '.$left.' >= '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_NOT_LESSER_OR_EQUAL)
		{
			return ' not '.$left.' <= '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_NOT_GREATER)
		{
			return ' not '.$left.' > '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_NOT_LESSER)
		{
			return ' not '.$left.' < '.$right.' ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_NOT_IN)
		{
			return ' not '.$left.' in ('.$right.') ' ;
		}
		else if ($operator == $this->DbTable->OPERATOR_NOT_LIKE)
		{
